<script lang="ts">
	import { vaccines, publicProductsStore } from '../../stores/publicProducts';
...
<svelte:head>
	<title>Vacunas | VAQ+</title>
	<meta
		name="description"
		content="Conoce nuestro programa de vacunas. Precios y información en VAQ+."
	/>
</svelte:head>
...
<div class="list-page">
	<div class="container">
		<header class="page-header">
			<h1>Vacunas</h1>
			<p class="page-intro">
				Programa completo de inmunización para todas las edades. Consulta precios e
				información de cada vacuna.
			</p>
		</header>
		...
		{:else if loaded && list.length === 0}
			<div class="empty-state">
				<p>No hay vacunas disponibles en este momento.</p>
				<a href="/#products" class="back-link">Volver al inicio</a>
			</div>
		{:else}
			<div class="cards-grid">
				{#each list as product (product.id)}
					<a href="/vaccines/{product.id}" class="product-card">
