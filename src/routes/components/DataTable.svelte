<script lang="ts">
	...
	$: pageNumbers = Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
		const n = i + 1;
		if (totalPages <= 5 || page <= 3) return n;
		if (page >= totalPages - 2) return totalPages - 4 + n;
		return page - 2 + n;
	});
		...
		<div class="table-actions">
			{#if selectable && selectedRows.length > 0}
				<button
					...
					on:click={() => dispatch('bulkDelete', selectedRows)}
				>
								...
								<td class="checkbox-cell" on:click|stopPropagation>
									<input
										...
										on:change={(e) => {
											const target = e.target;
											if (target && target instanceof HTMLInputElement) {
												handleRowSelect(row.id, target.checked);
											}
										}}
									/>
								</td>
